...
function InsightPost(props) {
  ...
  const addPostReaction = (postData) => {
    ...
    const data = {
      ...
      post_id,
      reaction: reaction === '1' ? 'dislike' : 'like',
    };
    showLoader();
    ...
    <div>
      {props.posts &&
        props.posts.map((prop, index, arr) => {
          if (prop.promotion_status === 'Insight') {
            return arr.length - 1 === index ? (
              <div
                key={prop.post_id}
                className={`mt30 top-content-bar container row ${
                  arr.length - 1 === index ? 'mb100' : 'mb100'
                }`}
                style={{ width: '700px' }}
              >
                <div className={prop.path ? 'col-md-6' : 'col-md-10'}>
                  <div className="p-3-0">
                    <img
                      style={{ width: '15%' }}
                      className="brad-40 h45"
                      src={
                        prop.profile_thumb
                          ? picUrl + prop.profile_thumb
                    ...
                    {ownPost(prop) ? (
                      ...
                      >
                        <span>
                          {prop.promotion_status === 'Insights'
                            ? 'Insights'
                            : 'Promote'}
                        </span>
                      </button>
                  ...
                  <div className="border-top">
                    <span className="mr20 red">
                      <i
                        className={`fa ${
                          ...
                          postDetailData.postData.like_status !== '1' &&
                          postDetailData.postData.like_status !== 1
                            ? 'fa-heart-o'
                            : 'fa-heart'
                        }`}
                        title="Like"
                        onClick={() => addPostReaction(prop)}
                      />
                    ...
                    <span className="mr20 gray">
                      <Moment fromNow>
                        {new Date(prop.date_created * 1000)}
                      </Moment>
                    </span>
                  </div>
                </div>
                <div className={prop.path ? 'col-md-6 p0' : 'col-md-2'}>
                  <div
                    className=""
                    onClick={() => openDetailsModal(prop, 'single-image')}
                  >
                    {prop.path !== null ? (
                      <img
                        onError={props.onCrash}
                        className="brad-10 post-image absolute"
                        src={picUrl + prop.path}
                        alt="authore pic"
                      />
                    ) : null}
                  </div>
                  <div className="float-right">
                    <DotCircleIcon
                      onClick={() => openPostOptonsModel(prop)}
                      className="profile-icons"
                    />
                    <ShareIcon onClick={sharePost} className="profile-icons" />
                    <LinkIcon
                      onClick={() => {
                        copy_to_clipboard(prop.caption + ' ' + prop.body);
                      }}
                      className="profile-icons"
                    />
                  </div>
            ...
            if (index === 0) {
              return <h1>No Insight Post Found</h1>;
            }
          }
        })}
      ...
      <SharePostModal
        ...
        showModel={showSharePostModel}
        hideShareModel={() => setShowSharePostModel(false)}
      />
      <PostOptionModal
